...
{
  Vector3 scale(4, 3, 5);
  Vector3 translation(5, 3, 1);
  Quaternion orientation(Degree(45), Degree(0), Degree(0));
  
  Matrix4 rot(orientation);
  ...
  Matrix4 scal(Matrix4::Identity);
  scal[0][0] = scale[0];
  scal[1][1] = scale[1];
  scal[2][2] = scale[2];
  
  REQUIRE(Matrix4::Transform(position, scale, orientation) == trans * rot * scale);
}
