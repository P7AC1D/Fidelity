...
TEST_CASE("CAMERA CULLING - ESSENTIAL CORRECTNESS TESTS", "[FRUSTUM][CULLING]")
{
  ComponentManager componentManager;

  SECTION("Point Containment - Basic Visibility")
  {
    // Create camera at (0,0,5) looking at origin
    auto [cameraGO, camera] = FrustumTestHelper::createCameraWithGameObject(
        &componentManager,
    ...
    // Create frustum using corrected construction
    Frustrum frustum(*camera);

    // Test critical points that were failing before the fix
    bool originVisible = frustum.contains(Vector3(0.0f, 0.0f, 0.0f));  // At origin - should be visible
    bool behindVisible = frustum.contains(Vector3(0.0f, 0.0f, 10.0f)); // Behind camera - should NOT be visible
    bool frontVisible = frustum.contains(Vector3(0.0f, 0.0f, -5.0f));  // In front - should be visible

    REQUIRE(originVisible == true);  // Critical fix verification
    REQUIRE(behindVisible == false); // Proper culling
    REQUIRE(frontVisible == true);   // Proper inclusion

    delete cameraGO;
  }

  SECTION("AABB Containment - Object Culling")
  {
    auto [cameraGO, camera] = FrustumTestHelper::createCameraWithGameObject(
        &componentManager,
        Vector3(0.0f, 0.0f, 5.0f),
    ...
    Aabb testBounds(Vector3(-0.5f, -0.5f, -0.5f), Vector3(0.5f, 0.5f, 0.5f));

    // Test object at origin (was failing before fix)
    TransformComponent originTransform;
    originTransform.setPosition(Vector3(0.0f, 0.0f, 0.0f));
    bool originObjectVisible = frustum.contains(testBounds, originTransform);

    // Test object behind camera
    TransformComponent behindTransform;
    behindTransform.setPosition(Vector3(0.0f, 0.0f, 10.0f));
    bool behindObjectVisible = frustum.contains(testBounds, behindTransform);

    // Test object in front of camera
    TransformComponent frontTransform;
    frontTransform.setPosition(Vector3(0.0f, 0.0f, -5.0f));
    bool frontObjectVisible = frustum.contains(testBounds, frontTransform);

    REQUIRE(originObjectVisible == true);  // Critical fix verification
    REQUIRE(behindObjectVisible == false); // Proper culling
    REQUIRE(frontObjectVisible == true);   // Proper inclusion

    delete cameraGO;
  }

  SECTION("Frustum Plane Normal Directions")
  {
    auto [cameraGO, camera] = FrustumTestHelper::createCameraWithGameObject(
        &componentManager,
        Vector3(0.0f, 0.0f, 5.0f),
    ...
    // Test that all planes have correct inward-pointing normals for origin point
    ...
    float leftDist = leftPlane.getSignedDistance(testPoint);
    float rightDist = rightPlane.getSignedDistance(testPoint);
    float topDist = topPlane.getSignedDistance(testPoint);
    float bottomDist = bottomPlane.getSignedDistance(testPoint);
    float nearDist = nearPlane.getSignedDistance(testPoint);
    float farDist = farPlane.getSignedDistance(testPoint);

    // All distances should be positive for a point inside the frustum
    REQUIRE(leftDist > 0.0f);
    REQUIRE(rightDist > 0.0f);
    REQUIRE(topDist > 0.0f);
    REQUIRE(bottomDist > 0.0f);
    REQUIRE(nearDist > 0.0f);
    REQUIRE(farDist > 0.0f);

    delete cameraGO;
  }

  SECTION("Side Plane Culling Accuracy")
  {
    auto [cameraGO, camera] = FrustumTestHelper::createCameraWithGameObject(
        &componentManager,
    ...
    // Test objects that should be outside the side planes
    bool leftOutside = frustum.contains(Vector3(-100.0f, 0.0f, -10.0f));   // Far left
    bool rightOutside = frustum.contains(Vector3(100.0f, 0.0f, -10.0f));   // Far right
    bool topOutside = frustum.contains(Vector3(0.0f, 100.0f, -10.0f));     // Far up
    bool bottomOutside = frustum.contains(Vector3(0.0f, -100.0f, -10.0f)); // Far down

    REQUIRE(leftOutside == false);
    REQUIRE(rightOutside == false);
    REQUIRE(topOutside == false);
    REQUIRE(bottomOutside == false);

    // Test object at center that should be visible
    bool centerVisible = frustum.contains(Vector3(0.0f, 0.0f, -10.0f));
    REQUIRE(centerVisible == true);
